@rem Copyright 2012 FEI. All rights reserved.
...
echo. "%MSDeployPath%\msdeploy.exe" -preSync:runCommand='net stop "Rem NServiceBus Host Service"',waitAttempts=60 -source:dirPath='%RootPath%' -dest:dirPath='%_Destination%' -verb:sync %_MsDeployAdditionalFlags% -postSync:runCommand="%_ArgDestinationType%\InstallAndStartRemNServiceBusHost.bat",waitAttempts=60
"%MSDeployPath%\msdeploy.exe" -preSync:runCommand='net stop "Rem NServiceBus Host Service"',waitAttempts=60 -source:dirPath='%RootPath%' -dest:dirPath=%_Destination% -verb:sync %_MsDeployAdditionalFlags% -postSync:runCommand="%_ArgDestinationType%\InstallAndStartRemNServiceBusHost.bat",waitAttempts=60
goto :eof
...
:usage
echo =========================================================
if not exist "%RootPath%Rem.Web.deploy-readme.txt" (
...
start notepad "%RootPath%Rem.Web.deploy-readme.txt"
)
echo =========================================================